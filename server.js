// server.js
// Minimal backend for Withdrawal + Receipts (in-memory storage)
...
const receipts = new Map();    // id -> receipt record
 ...
 * Body: { chain, address, amount, publicCode, requirementConfirmed }
 * Returns: { ok, receiptId, url }
 */
...
// Optional: JSON lookup for a receipt (useful for AJAX polling if needed)
...
// HTML receipt page (professional look, print‑friendly)
      ...
      <div class="sheet bg-[#0b1020] border border-white/10 rounded-xl overflow-hidden shadow-2xl">
        <div class="flex items-center justify-between p-5 bg-[#10162a] border-b border-white/10">
          <div class="flex items-center gap-3">
            ...
            <div>
              <div class="text-lg font-semibold">${BUSINESS_NAME}</div>
              <div class="text-xs text-gray-400">${SUPPORT_EMAIL}</div>
            </div>
          </div>
        ...
        <div class="p-6 space-y-6">
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
            <div><div class="text-gray-400">Created</div><div>${createdAt}</div></div>
            <div><div class="text-gray-400">Network</div><div class="uppercase">${r.chain}</div></div>
            <div><div class="text-gray-400">Requester IP</div><div class="font-mono">${r.ip || "-"}</div></div>
          </div>
          ...
          <div>
            ...
            <table class="w-full text-sm border border-white/10 rounded-lg overflow-hidden">
              <thead class="bg-[#0f162d]">
                <tr><th class="text-left px-3 py-2">Field</th><th class="text-left px-3 py-2">Details</th></tr>
              </thead>
              <tbody>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
            ...
            <div class="text-sm">
              ...
              <div class="flex items-center gap-2">
                ...
                <button class="no-print bg-sky-600 hover:bg-sky-500 px-3 py-2 rounded text-xs"
                  onclick="navigator.clipboard.writeText(document.getElementById('link').value).then(()=>alert('Link copied'))">
                  Copy
                </button>
              </div>
...
// Simple admin list — /admin?key=YOUR_ADMIN_KEY
    ...
    <table class="w-full text-sm border">
      <thead>
        <tr class="bg-gray-100">
          <th class="p-2">Time</th><th>Chain</th><th>Address</th><th>Amount</th>
          <th>Public Code</th><th>Requirement</th><th>Receipt</th><th>Status</th><th>IP</th>
        </tr>
      </thead>
      <tbody>
        ${
...
// Default route -> your withdrawal page
app.get("/", (_req, res) => {
